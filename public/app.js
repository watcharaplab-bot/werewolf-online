...
function renderHunter(){
  ...
  if(me.id!==state.players.find(p=>p.id===me.id)?.id){return}
...
socket.on("privateResult",r=>{if(r.type==="seer")$("private").innerHTML=`<div class="notice">🔮 ${r.target} เป็น <b>${roles[r.role]?.[1]||r.role}</b> — ${r.isWolf?"🐺 เป็นหมาป่า":"ไม่ใช่หมาป่า"}</div>`});
